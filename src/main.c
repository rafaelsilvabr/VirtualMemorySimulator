...
int main() {
    ...
    for(int frames = MIN_FRAMES; frames <= MAX_FRAMES; frames += FRAME_STEP) {
        ...
        for(int exp = 0; exp < NUM_EXPERIMENTS; exp++) {
            Statistics fifo_stats = simulate_memory(FIFO, frames, NUM_REFERENCES);
            Statistics sc_stats = simulate_memory(SECOND_CHANCE, frames, NUM_REFERENCES);
            Statistics clock_stats = simulate_memory(CLOCK, frames, NUM_REFERENCES);
